...
import test.java.awt.event.SpellWreckedKeyEvent;
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<keyboard.length; i++){
			for(int j=0; j<keyboard[i].length; j++){
				...
				KeyEvent e = new SpellWreckedKeyEvent(frame, KeyEvent.RESERVED_ID_MAX + 1, System.currentTimeMillis(), 0, KeyEvent.VK_UNDEFINED, keyboard[i][j]);
		...
		for(int i=0; i<shiftKeyboard.length; i++){
			for(int j=0; j<shiftKeyboard[i].length; j++){
				...
				KeyEvent e = new SpellWreckedKeyEvent(frame, KeyEvent.RESERVED_ID_MAX + 1, System.currentTimeMillis(), KeyEvent.SHIFT_DOWN_MASK, KeyEvent.VK_UNDEFINED, shiftKeyboard[i][j]);
				char output = e.getKeyChar();
				if(input != output){
					numSpellwrecks++;
					System.out.println("Detected Spellwreck (" + numSpellwrecks + "): Input: " + input + ", Output: " + output + "\n");
