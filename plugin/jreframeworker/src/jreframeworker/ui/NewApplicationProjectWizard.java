...
public class NewApplicationProjectWizard extends Wizard implements INewWizard {
	...
	private NewApplicationProjectPage page;

	public NewApplicationProjectWizard(String startApplicationPath) {
		page = new NewApplicationProjectPage("Create JReFrameworker Application Project");
		String projectName = new File(startApplicationPath).getName();
		...
		this.setWindowTitle("Create JReFrameworker Application Project");
	}
	...
	public boolean performFinish() {
		...
		final String targetJarName = page.getTargetJarName();
		...
		IRunnableWithProgress j = new IRunnableWithProgress() {
			...
			public void run(IProgressMonitor monitor) {
				IStatus result = null;
				try {
					result = JReFrameworker.createProject(projectName, projectLocation, monitor, targetJarName, false);
				} catch (Throwable t) {
					String message = "Could not create JReFrameworker application project. " + t.getMessage();
